...
function sortPaddockTypeByTotalArea() {
    ...
    //ordenar cultivos
    paddockTotalArea.sort((a, b) => b.total - a.total);

    //mappear resultado con nombres
    let result = paddockTotalArea.map(paddock => paddockType[paddock.id - 1].name);
    return result;
...
function sortFarmManagerByAdminArea() {
    ...
    //mappear resultado con nombres
    let result = paddockTotalArea.map(paddock => paddockManagers[paddock.id - 1].name);
    return result;
}
...
function farmManagerPaddocks() {
    // tu código aquí
    let managers = paddockManagers.slice();
